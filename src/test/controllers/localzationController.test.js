...
describe('LocalizationController', () => {
  test('Quando recebe request com 2 ou mais endereços válidos, deve retornar uma resposta de sucesso', async () => {
    ...
    const localizationController = require('../../controllers/localizationController')(localizationServiceMock);

    const controllerResponse = await localizationController.post(validParsedRequestBody, resMock);
  ...
  test('Quando, ao fazer o request para a api do google, esta retorna 400. Deve retornar um erro com statusCode 400', async () => {
    ...
    return Promise.all([
      expect(response.statusCode).toBe(400),
      expect(response.message).toBe('Erro ao realizar request para o a API do Google.'),
    ]);
  });
  ...
  test('Quando, ao fazer o request para a api do google, esta retornar 403. Deve retornar um erro com statusCode 500', async () => {
    ...
    return Promise.all([
      ...
      expect(response.message).toBe('Ocorreu um erro interno ao tentar processar os endereços'),
    ]);
  });
});
