...
describe('LocalizationService', () => {
  test('resolveAddressesToGeoCoding - Quando recebe uma lista de endereços válida, retorna uma lista com as localizações', async () => {
    ...
    const localizationService = require('../../services/localizationService')(integrationServiceImplValidMock);

    const response = await localizationService.resolveAddressesToGeoCoding(validAddresses);
  ...
  test('resolveAddressesToGeoCoding - Quando recebe uma lista sem enredeços, retorna um erro', () => {
    ...
    return expect(localizationService.resolveAddressesToGeoCoding([])).rejects.toThrowError(
      'Não há endereços para serem processados',
    );
  });
  ...
  test('resolveAddressesToGeoCoding - Quando a chamada da integração que busca a geolocalização dá erro, deve retornar o erro', () => {
    ...
    const localizationService = require('../../services/localizationService')(integrationServiceImplWithErrorMock);

    return expect(localizationService.resolveAddressesToGeoCoding(validAddresses)).rejects.toThrow();
  });
  ...
  test('processDistanceBeetweenCodedAddresses - Quando recebe uma lista de endereços geolocalizados válida, retorna uma lista com os endereços e com as distâncias ordenadas entre cada endereço', async () => {
    ...
    const flattened = validResponseFromResolveAddressesToGeoCoding.flatMap((it) => it && it.results);

    const response = await localizationService.processDistanceBeetweenCodedAddresses(flattened);

    return Promise.all([expect(response).toEqual(expectedResponseFromProcessDistance)]);
  });
  ...
  test('processDistanceBeetweenCodedAddresses - Quando recebe uma lista de endereços geolocalizados sem campo geometry, retorna um erro', async () => {
    ...
    const localizationService = require('../../services/localizationService')(addressesGeoCodedWithoutGeometryField());
    const flattened = validResponseFromResolveAddressesToGeoCoding.flatMap((it) => it && it.results);

    return expect(() => localizationService.processDistanceBeetweenCodedAddresses(flattened)).toThrowError(
  ...
  test('processDistanceBeetweenCodedAddresses - Quando recebe uma lista de endereços geolocalizados sem campo location, retorna um erro', async () => {
    ...
    const localizationService = require('../../services/localizationService')(addressesGeoCodedWithoutLocationField());
    const flattened = validResponseFromResolveAddressesToGeoCoding.flatMap((it) => it && it.results);
