...
const googleAPIService = (httpRequestImpl, baseEndpoint, apiKey) => {
    return {
        getGeoLocalization: (address) => {
            console.log(
                'GoogleAPIService - Iniciando a construção do request para buscara as geolocalizações do endereço',
            );
            console.debug(address);

            const { city, state, number, street, postalCode, neighborhood } = address;
